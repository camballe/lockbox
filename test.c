...
int main()
{
    ...
    printf("'%s'\n ->", text);
    encrypted = lockboxEncrypt(lockbox, (int8 *)text, textSize);
    printBinaryData(encrypted, textSize);

    lockboxUninit(lockbox);
    ...
    decrypted = lockboxDecrypt(lockbox, encrypted, textSize);
    printf("    ->'%s'\n", decrypted);
    lockboxUninit(lockbox);
